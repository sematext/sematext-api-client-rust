...
fn get_client() -> LogsAppApiClient<HttpConnector<GaiResolver>> {
    ...
    LogsAppApiClient::new(Arc::new(conf))
}
/*
*/
...
async fn create_logsene_application_test() {
    let api_client = get_client();
    let value = json!(/*Put test json here*/);
    let body: CreateAppInfo = serde_json::from_value(value).unwrap();
    let response: AppsResponse = api_client.create_logsene_application(body).await.unwrap();
}
