...
pub struct AwsSettingsControllerApiClient<C: hyper::client::connect::Connect + Clone + Send + Sync> {
    configuration: Arc<configuration::Configuration<C>>,
}

impl<C: hyper::client::connect::Connect + Clone + Send + Sync + 'static> AwsSettingsControllerApiClient<C> {
    pub fn new(configuration: Arc<configuration::Configuration<C>>) -> AwsSettingsControllerApiClient<C> {
...
pub trait AwsSettingsControllerApi {
    async fn update_using_put(&self, body: crate::models::CloudWatchSettings, app_id: i64) -> Result<CloudWatchSettingsResponse, Error<serde_json::Value>>;
}

#[async_trait::async_trait]
impl<C: hyper::client::connect::Connect + Clone + Send + Sync + 'static>AwsSettingsControllerApi for AwsSettingsControllerApiClient<C> {
 ///
 ...
 /// # Arguments
 ///
 /// * `body` - Update App&#x27;s AWS CloudWatch settings
 /// Applicable only for AWS Apps
 ///
 /// * `app_id` - Update App&#x27;s AWS CloudWatch settings
 /// Applicable only for AWS Apps
 ///
    async fn update_using_put(&self, body: crate::models::CloudWatchSettings, app_id: i64) -> Result<CloudWatchSettingsResponse, Error<serde_json::Value>> {
        ...
        let uri_str = format!("{}users-web/api/v3/apps/{appId}/aws{}", configuration.base_path, query_string, appId=app_id);
